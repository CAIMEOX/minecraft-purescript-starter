...
gulp.task("compile_pure", (cb) => {
  exec("spago build", (err, stdout, stderr) => {
    cb(err);
  });
});
...
const del_build_scripts = del_gen([join(output, outputPath)]);
...
function watch() {
  return gulp.watch(
    [join(behPath, "**/*"), join(resPath, "**/*"), join(scriptsPath, "**/*")],
